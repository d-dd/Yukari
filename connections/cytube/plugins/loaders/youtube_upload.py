...
# Google and used according to terms described in the Apache 2.0 License.
...
def resumable_upload(insert_request, options):
  ...
  while response is None:
    try:
      ...
      if 'id' in response:
#          print ("'%s' (video link: http://youtu.be/%s) was successfully uploaded." % 
#                 (options.title.encode('utf8'), response['id']))
          print 'DONE %s' % response['id']
      else:
